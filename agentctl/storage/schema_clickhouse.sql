...
SELECT
    ...
    toUInt64OrZero(attributes['measure.prompt_tokens'])        AS prompt_tokens,
    toUInt64OrZero(attributes['measure.completion_tokens'])    AS completion_tokens,
...
SELECT
    ...
    canary_arm,
    toStartOfHour(timestamp)                              AS hour,
    toUInt64OrZero(attributes['measure.frames_out'])      AS frames_out,
    toUInt64OrZero(attributes['measure.shadow_sent'])     AS shadow_sent,
    toUInt64OrZero(attributes['measure.shadow_dropped'])  AS shadow_dropped,
...
SELECT
    ...
    toUInt64OrZero(attributes['measure.bytes'])      AS bytes_total,
    toUInt64OrZero(attributes['measure.frames_out']) AS frames_total
FROM agentctl.otel_spans
WHERE name = 'gateway.stream.metrics';
